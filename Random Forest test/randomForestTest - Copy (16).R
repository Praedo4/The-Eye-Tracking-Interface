...
for(test_index in (0:31)){
  ...
  cur_balancing <- balancing_list[floor(test_index/8) %% 2 + 1]
  
  parameters <- c('100', cur_model, cur_scoring_function, cur_balancing,'null')
  if((floor(test_index / 16)%% 2) == 0){
  ...
  if((floor(test_index / 16)%% 2) == 0){
    parameters <- c('100',cur_model, cur_scoring_function, cur_balancing, sprintf("4_results_0.3/%s_%s_%s_%s.txt",cur_model,cur_scoring_function,cur_search,cur_balancing))
  }
  else{
    parameters <- c('100',cur_model, cur_scoring_function, cur_balancing, sprintf("4_results_0/%s_%s_%s_%s.txt",cur_model,cur_scoring_function,cur_search,cur_balancing))
  }
